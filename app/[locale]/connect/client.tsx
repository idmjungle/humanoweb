...
const ContactPage = () => {
  ...
  return (
    <div className="p-6">
      ...
      <h2 className="my-4 font-bold text-base md:text-3xl tracking-tighter leading-5">
        {t("description")}
      </h2>
      <form
        onSubmit={handleSubmit}
        className={`p-3 lg:p-8 grid lg:grid-cols-2 xl:grid-cols-3 text-base lg:text-xl bg-humano-light ${
          sending || success ? "rounded-full" : "rounded-4xl"
        } gap-4 transition-all duration-1500`}
      >
        {sending && (
          <div className="text-9xl font-bold animate-pulse text-humano-dark tracking-tighter leading-[0.8]">
            {t("sending")}
          </div>
        )}
        {success && (
          <div className="text-9xl font-bold text-humano-dark tracking-tighter leading-[0.8] w-full">
            {t("success")}
          </div>
        )}
        <div
          className={`space-y-3 md:space-y-5 ${
            (sending || success) && "hidden"
          }`}
        >
          <div>
            ...
            <input
              ...
              className={`w-full rounded-full py-2.5 px-4 ${
                error && message.find((m) => m.path[0] === "name")
                  ? "text-humano-light bg-humano-red"
                  : "text-humano-light bg-humano-dark"
              } border border-humano-dark focus:border-humano-dark focus:bg-humano-yellow focus:text-humano-dark outline-0 transition-all`}
            />
            {error && message.find((m) => m.path[0] === "name") && (
          ...
          <div>
            ...
            <input
              ...
              id="email"
              name="email"
              className={`w-full py-2.5 rounded-full px-4 ${
                error && message.find((m) => m.path[0] === "email")
                  ? "text-humano-light bg-humano-red"
                  : "text-humano-light bg-humano-dark"
              } border border-gray-200 focus:border-humano-dark focus:bg-humano-yellow focus:text-humano-dark outline-0 transition-all`}
            />
            {error && message.find((m) => m.path[0] === "email") && (
              <div className="text-humano-red text-lg">
